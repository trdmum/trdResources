    ...
    var filesList = document.getElementById(id);    
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if(files[i].name.endsWith("pdf")){
          ...
          li.innerHTML='<div class="cd-accordion__label cd-accordion__label--icon-img"><i class="fa fa-trash-o" style="font-size:1.5em;cursor:pointer;" onclick="delPopUp2('+filePath+');"></i><span  style="cursor:pointer;" onclick="viewFile('+filePath+');">'+files[i].name+'</span></div>';
        }        
        ul.appendChild(li);
      }
...
function viewFile2(filePath){
   ...
   dbx.sharingCreateSharedLink({path:filePath })
        .then(function(res) {
          ...
          window.open(downloadLinkNew,'_blank');
        })
        .catch(function(error) {
          console.error(error);          
        });
...
function viewFile(filePath){
    ...
    dbx.sharingGetSharedLinks({path:filePath })
        .then(function(res) {
          // console.log('response', res)                                        
          var downloadLinkNew=res.result.links[0].url;
          // location.href=downloadLinkNew;
          window.open(downloadLinkNew,'_blank');
        })
        .catch(function(error) {
